...
from espnet2.VC_SRC.Model_component.layers import ConvNorm, LinearNorm,Tacotron1_prenet,StatisticPool
...
class mvemb_condition(torch.nn.Module):
    def __init__(self,num_of_spk,emb_dim):
        ...
        weight=torch.cat([torch.zeros((num_of_spk,emb_dim)),torch.ones((num_of_spk,emb_dim))],dim=1)
        self.speaker_embedding=torch.nn.Embedding(num_of_spk, emb_dim*2,_weight=weight)
...
class VC_model(nn.Module):
    def __init__(self, hparams):
        ...
        self.speaker_condition_layer=mvemb_condition(hparams.num_of_spk,hparams.speaker_emb_dim)

        self.dim_reduction_layer=torch.nn.Linear(hparams.prenet_dim,hparams.encoder_embedding_dim, bias=True)
...
if __name__ == '__main__':
    ...
    mel_loss,mel_loss_final,mel_output_final = m.forward(bn_feat, seq_length,spk_id,target_mel)
    out,cc = m.inference(bn_feat, seq_length,spk_id)

    print(mel_output_final.mean(),mel_output_final.std())
