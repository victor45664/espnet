...
from espnet2.VC_SRC.Model_component.layers import ConvNorm, LinearNorm,Tacotron1_prenet,attention_contition
...
#使用attention 的weight作为spk embedding
...
if __name__ == '__main__':
    ...
    # dictionary_size=11
    # spk_emb_dim=33
    # weight = torch.softmax(torch.rand((B, length, dictionary_size)),dim=2)
    # spkemb = torch.rand((B, dictionary_size*spk_emb_dim))
    # atten=torch.matmul(weight, spkemb.view(B,dictionary_size,spk_emb_dim))
    # b=1
    #
    # sB=1
    # slength=5
    # sd=22
    #
    # gg=(weight[sB,slength,:]*spkemb.view(B,dictionary_size,spk_emb_dim)[sB,:,sd]).sum()
    # assert gg-atten[sB, slength, sd]<=0.0001
    # b=1
    #
    #
