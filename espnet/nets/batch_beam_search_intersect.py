...
from espnet.nets.beam_search_intersect import BeamSearch
from espnet.nets.beam_search_intersect import Hypothesis
import numpy as np
...
def intersect_fusion(acoustic_score, lm_score,topk=100):
    ...
    if intersection.shape[0]==0:  #no intersect using normal fusion method
        return fused_score
    else:
        intersected_fused_score=fused_score.new_ones(fused_score.shape[0])*(-1000)  #only the intersected symbols have socre anything else is set to negitive inf
        intersected_fused_score[intersection]=fused_score[intersection]
    return intersected_fused_score
...
class BatchBeamSearch(BeamSearch):
    ...
    def search(self, running_hyps: BatchHypothesis, x: torch.Tensor) -> BatchHypothesis:
        ...
        acoustic_score=weighted_scores-self.weights["lm"]*scores["lm"]

        for i in range(int(acoustic_score.shape[0])):
            weighted_scores[i]=intersect_fusion(acoustic_score[i],self.weights["lm"]*scores["lm"][i],self.intersection_topk)  #这里已经乘以了系数，函数内可以直接融合
