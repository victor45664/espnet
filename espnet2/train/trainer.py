...
class Trainer_ilme_adl(Trainer):
    ...
    def train_one_epoch(
        ...
        for iiter, (_, batch) in enumerate(
            ...
            #先进行LAS 前向后向，然后ILM 前向后向，可以节约显存
            if cls.do_las_when_adl_loss_is_low==True or cls.decoder_parameter_switch==1:
                #LAS forward
                with autocast(scaler is not None):
                    with reporter.measure_time("forward_time"):
                        retval = model(**batch)
...
class Trainer_ilme_unadl(Trainer):
    ...
    def train_one_epoch(
        ...
        if isinstance(model, torch.nn.DataParallel):
            ...
            model_adl_begin_loss = model.adl_begin_loss
            model_decoder_parameter = model.decoder.decoder_parameter
            model_ilme_parameter = model.decoder.ilme_parameter
            model_ctc_weight = model.ctc_weight
            model_adl_factor = model.adl_factor
            ...
            if cls.do_las_when_adl_loss_is_low==True or cls.decoder_parameter_switch==1:
                ...
                with autocast(scaler is not None):
                    with reporter.measure_time("forward_time"):
                        ...
                        #   b. tuple or list type
                            ...
                            ilm_loss, stats, weight = retval
                            optim_idx = None

                    stats = {k: v for k, v in stats.items() if v is not None}
                    if ngpu > 1 or distributed:
                        # Apply weighted averaging for loss and stats
                        ilm_loss = (ilm_loss * weight.type(ilm_loss.dtype)).sum()
                    ...
                    if distributed:
                        # NOTE(kamo): Multiply world_size because DistributedDataParallel
                        # automatically normalizes the gradient by world_size.
                        ilm_loss *= torch.distributed.get_world_size()
                ...
                with reporter.measure_time("backward_time"):
                    if scaler is not None:
                        ...
                        # Backward ops run in the same dtype autocast chose
                        # for corresponding forward ops.
                        scaler.scale(ilm_loss).backward()
                    else:
                        ilm_loss.backward()
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time_ilm"):
                    ...
                    #   b. tuple or list type
                        ...
                        ilm_loss,unadl_loss, stats, weight = retval
                        optim_idx = None
                ...
                if ngpu > 1 or distributed:
                    # Apply weighted averaging for loss and stats
                    ilm_loss = (ilm_loss * weight.type(ilm_loss.dtype)).sum()
                    unadl_loss = (unadl_loss * weight.type(unadl_loss.dtype)).sum()
                ...
                if distributed:
                    ...
                    # automatically normalizes the gradient by world_size.
                    ilm_loss *= torch.distributed.get_world_size()
                    unadl_loss *= torch.distributed.get_world_size()

                ilm_loss /= accum_grad
                unadl_loss /= accum_grad
            ...
            with reporter.measure_time("backward_time_ilm"):
                ...
                for i in range(len(model_decoder_parameter)):
                    hooks.append(
                        model_decoder_parameter[i].register_hook(
                            lambda grad: grad * 0)
                        # we only calculate grad for ilme parameters so we set the grad of other parameters in decoder to zero
                    )
                ...
                if scaler is not None:
                    ...
                    scaler.scale(ilm_loss).backward(retain_graph=bool(cls.decoder_parameter_switch))# 只有ilm loss足够低的时候才进行adl训练,才需要保留graph
                else:
                    ilm_loss.backward(retain_graph=bool(cls.decoder_parameter_switch))   #this backward only affect ilme parameters

                for i in range(len(hooks)):
                    hooks[
                        i].remove()

            if cls.decoder_parameter_switch==1: #只有ilm loss足够低的时候才进行adl训练
                with reporter.measure_time("backward_time_unadl"):

                    hooks = []
                    for i in range(len(model_ilme_parameter)):
                        hooks.append(
                            model_ilme_parameter[i].register_hook(
                                lambda grad: grad * 0)
                            # we only calculate grad for decoder parameters so we set the grad of ilme parameters in decoder to zero
                        )
                    unadl_loss = unadl_loss * (1 - model_ctc_weight) * model_adl_factor  # 对抗loss 必须和attention loss成比例
                    # 注意与普通的adl不同，这里的model_adl_factor不需要负号
                    if scaler is not None:
                        # Scales loss.  Calls backward() on scaled loss
                        # to create scaled gradients.
                        # Backward passes under autocast are not recommended.
                        # Backward ops run in the same dtype autocast chose
                        # for corresponding forward ops.
                        scaler.scale(unadl_loss).backward()
                    else:
                        unadl_loss.backward()   #this backward only affect ilme parameters
