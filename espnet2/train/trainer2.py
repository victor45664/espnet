...
class Trainer_ilmt(Trainer):
    @classmethod
    def train_one_epoch(
            cls,
            model: torch.nn.Module,
            iterator: Iterable[Tuple[List[str], Dict[str, torch.Tensor]]],
            ilmt_iterator: Iterable[Tuple[List[str], Dict[str, torch.Tensor]]],
            optimizers: Sequence[torch.optim.Optimizer],
            schedulers: Sequence[Optional[AbsScheduler]],
            scaler: Optional[GradScaler],
            reporter: SubReporter,
        ...
        grad_noise = options.grad_noise
        accum_grad = options.accum_grad
        ilmt_accum_grad = options.ilmt_accum_grad
        ilmt_loss_weight = options.ilmt_loss_weight
        grad_clip = options.grad_clip
        grad_clip_type = options.grad_clip_type
        log_interval = options.log_interval
        no_forward_run = options.no_forward_run
        ...
        for iiter, (_, batch) in enumerate(
            ...
            fake_speech=batch["speech"].new_zeros(1)
            del batch  # save cuda mem
            for ilmt_train in range(ilmt_accum_grad):  #ilmt 训练循环，每次主循环中ilmt都需要循环ilmt_accum_grad次，
                                                        #因此实际累加的梯度是ilmt_accum_grad*accum_grad
                _,ilmt_batch = ilmt_iter.next()

                ilmt_batch=to_device(ilmt_batch, "cuda" if ngpu > 0 else "cpu")
                ilmt_batch["speech"]=fake_speech
                ilmt_batch["speech_lengths"]=-1
                with autocast(scaler is not None):

                    retval = model(**ilmt_batch,forward_ilm=True)
                    ...
                    loss /= accum_grad*ilmt_accum_grad



                loss=loss*ilmt_loss_weight
                if scaler is not None:
                    # Scales loss.  Calls backward() on scaled loss
                    # to create scaled gradients.
                    # Backward passes under autocast are not recommended.
    ...
    def run(
        ...
        schedulers: Sequence[Optional[AbsScheduler]],
        train_iter_factory: AbsIterFactory,
        ilmt_iter_factory: AbsIterFactory,
        valid_iter_factory: AbsIterFactory,
        plot_attention_iter_factory: Optional[AbsIterFactory],
        trainer_options,
        distributed_option: DistributedOption,
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            with reporter.observe("train") as sub_reporter:
                all_steps_are_invalid = cls.train_one_epoch(
                    ...
                    schedulers=schedulers,
                    iterator=train_iter_factory.build_iter(iepoch),
                    ilmt_iterator=ilmt_iter_factory.build_iter(iepoch),
                    reporter=sub_reporter,
                    scaler=scaler,
                    summary_writer=summary_writer,
                    options=trainer_options,
