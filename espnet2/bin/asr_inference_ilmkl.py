...
#这个解码器使用AED的输出和ILM输出的KL散度作为所有候选的分数
...
class Ilm_KL_Scorer(BatchScorerInterface):
    ...
    def batch_score(self,*argv,**kwargs):
        ...
        aed_score,aed_state=self.decoder.batch_score(ys,[i[0] for i in state],xs, **kwargs)
        ilm_score,ilm_states=self.decoder.batch_score_ilm(ys,[i[1] for i in state],xs, **kwargs)
        kd_loss = 5000*torch.mean(self.kl_loss(ilm_score, aed_score),dim=1)
        punishment = torch.ones(aed_score.shape,dtype=aed_score.dtype, device=aed_score.device)
        out_score=punishment*kd_loss.view(-1,1)

        return out_score,list(zip(aed_state,ilm_states))
...
class Speech2Text:
    ...
    def __init__(
        ...
        if ilm_weight=="0.0":
            ...
            ilm_kl_scorer = Ilm_KL_Scorer(decoder)
        scorers.update(
            decoder=decoder,
            ilm=ilm_kl_scorer,
            ctc=ctc,
