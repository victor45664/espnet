...
from espnet.nets.batch_beam_search_extract_allhyp import BatchBeamSearch
...
class Speech2Text:
    ...
    def __call__(
        ...
        gridsearch_results=[]
        assert len(self.gridsearchweights)==1,"don't support grid search"
        for weights in self.gridsearchweights:
            # c. Passed the encoder result and the beam search
            self.beam_search.weights=weights   #替换新的weights
            nbest_hyps,all_hyps = self.beam_search(
                x=enc[0], maxlenratio=self.maxlenratio, minlenratio=self.minlenratio
            )
            nbest_hyps = nbest_hyps[: self.nbest]

            filtered_hyps=[]#过滤掉 重复的序列
            for i in range(len(all_hyps)-1):
                hyps_this_step=all_hyps[i]  #当前step的所有hyps
                hyps_next_step=all_hyps[i+1]  #当前step的所有hyps
                for j in range(hyps_this_step.shape[0]):
                    is_dup=False
                    for k in range(hyps_next_step.shape[0]):
                        if hyps_this_step[j][-1]==hyps_next_step[k][-2]:
                            is_dup=True   #说明这个是重复的，跳过
                            break
                    if not is_dup:
                        filtered_hyps.append(hyps_this_step[j])
            filtered_hyps_text=[]
            for i in range(len(filtered_hyps)):
                token_int = filtered_hyps[i][1:-1].tolist() if filtered_hyps[i][-1]==4999 else filtered_hyps[i][1:].tolist()
                token_int = list(filter(lambda x: x != 0, token_int))
                token = self.converter.ids2tokens(token_int)
                text = self.tokenizer.tokens2text(token)
                filtered_hyps_text.append(text)
            results = []
        ...
        return gridsearch_results,filtered_hyps_text
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                t0=time.time()
                gridesearchresults,all_hyp = speech2text(**batch)
            ...
            for ri in range(len(speech2text.gridsearchweights)):
                ...
                for i in range(len(all_hyp)):
                    writer["all_hyp"][key+"_hyp"+str(i)] = all_hyp[i]
