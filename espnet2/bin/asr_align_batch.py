...
def ctc_align(
    ...
    **kwargs,
    ...
    wav_scp=read_2column_text(wav_scp_path)
    text_scp=read_2column_text(text_scp_path)


    keys=wav_scp.keys()
    with DatadirWriter(output_dir) as writer:
        scp_writer = writer["align.scp"]
        for key in keys:
            wav_path=wav_scp[key]
            speech, fs = soundfile.read(wav_path)
            text=text_scp[key]
            segments = aligner(speech=speech, text=text, fs=fs, name=key)
            scp_writer[key] = os.path.join(output_dir, key + ".npy")
            np.save(os.path.join(output_dir, key + ".npy"), segments.timings)
            gg=1
