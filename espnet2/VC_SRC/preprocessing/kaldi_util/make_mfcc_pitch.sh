...
ppg_extractor/validate_data_dir.sh --no-text --no-feats $data || exit 1;
...
for n in $(seq $nj); do
  ...
  # ppg_extractor/create_data_link.pl for more info.
  ppg_extractor/create_data_link.pl $mfcc_pitch_dir/raw_mfcc_pitch_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  ppg_extractor/split_scp.pl $data/segments $split_segments || exit 1;
  ...
  ppg_extractor/split_scp.pl $scp $split_scps || exit 1;
...
if [ $nf -ne $nu ]; then
  echo "$0: It seems not all of the feature files were successfully procesed" \
       "($nf != $nu); consider using ppg_extractor/fix_data_dir.sh $data"
fi
