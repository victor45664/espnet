...
if [ -f $data/feats.scp ] && [ $nu -ne $nf ]; then
  ...
  echo "**  use ppg_extractor/fix_data_dir.sh $data to fix this."
fi
if [ -f $data/text ] && [ $nu -ne $nt ]; then
  echo "** split_data.sh: warning, #lines is (utt2spk,text) is ($nu,$nt); you can "
  echo "** use ppg_extractor/fix_data_dir.sh to fix this."
fi
...
ppg_extractor/split_scp.pl $utt2spk_opt $data/utt2spk $utt2spks || exit 1

for n in `seq $numsplit`; do
  dsn=$data/split${numsplit}${utt}/$n
  ppg_extractor/utt2spk_to_spk2utt.pl $dsn/utt2spk > $dsn/spk2utt || exit 1;
done
...
for f in feats.scp text vad.scp utt2lang $maybe_wav_scp utt2dur utt2num_frames; do
  if [ -f $data/$f ]; then
    ppg_extractor/filter_scps.pl JOB=1:$numsplit \
...
for f in spk2gender spk2warp cmvn.scp; do
  if [ -f $data/$f ]; then
    ppg_extractor/filter_scps.pl $warning_opt JOB=1:$numsplit \
